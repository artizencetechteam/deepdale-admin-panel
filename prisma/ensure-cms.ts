...
function imageAsset(label: string, accent: string) {
...
async function ensureHeroContent() {
  ...
  const [heroTabCount, promptTemplateCount] = await Promise.all([
    prisma.heroTab.count({ where: { heroContentId: 1 } }),
    prisma.heroPromptTemplate.count({ where: { heroContentId: 1 } })
  ]);
...
async function ensureSupportFormConfig() {
  await prisma.supportFormConfig.upsert({
    where: { id: 1 },
    update: {},
    create: {
      id: 1,
  ...
  const checkItemCount = await prisma.supportFormCheckItem.count({
...
async function ensureSectionStates() {
  const existingStates = await prisma.sectionState.findMany({
    select: {
      key: true,
      sortOrder: true
    }
  ...
  const missingKeys = SECTION_KEYS.filter((key) => !existingKeys.has(key));
  ...
  const nextSortOrder =
    existingStates.reduce(
      (highest, state) => Math.max(highest, state.sortOrder),
      -1
    ) + 1;
...
async function ensureCollections() {
  // Product Cards
  if ((await prisma.productCard.count()) === 0) {
  ...
  // Voice Scenarios
  if ((await prisma.voiceScenario.count()) === 0) {
  ...
  // Automation Engines
  if ((await prisma.automationEngine.count()) === 0) {
  ...
  // Capability Cards
  if ((await prisma.capabilityCard.count()) === 0) {
  ...
  // Product Features
  if ((await prisma.productFeature.count()) === 0) {
  ...
  // Caller Profiles
  if ((await prisma.callerProfile.count()) === 0) {
  ...
  // Mega Menu Items
  if ((await prisma.megaMenuItem.count()) === 0) {
...
async function main() {
  ...
  console.log(
    `Ensured Deepdale CMS baseline data and ${createdSectionStates} missing section state(s).`
  );
}
